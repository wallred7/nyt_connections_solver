...
def run_clustering(start_idx: int, end_idx: int, debug_mode: bool = False):
    ...
    if not debug_mode:
        mlflow.log_text("modle", "constrained kmeans")  # "scikit kmeans" | "Custom Implementation"
        mlflow.autolog()
...
def main(start_idx: int = 0, 
    ...
    if debug:
        ...
        with disable_mlflow_logging():
            results = run_clustering(start_idx=0, end_idx=3, debug_mode=True)
    else:
        # Run with MLflow logging
        with mlflow.start_run():
            results = run_clustering(start_idx=0, end_idx=3, debug_mode=False)
